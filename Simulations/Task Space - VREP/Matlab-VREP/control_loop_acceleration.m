function [jointSpeed,jointAcceleration, error, w, jacobA] = control_loop(trajectoryStep,poseActual, stepSize, clientID, object, functionCall, vrep, endEffectorVelocity, normalVector, forceReading, forceControl,torqueVector, maxSpeed, previousTrajectory, previousJacobian, jointSpeed)% posePrevious)
    %% Error Calculation
    %poseActual
    %trajectoryStep
...
%     end
    ...
    dJacobA = (jacobA - previousJacobian)/stepSize;
    
    errorSpeed = xdDesired - endEffectorVelocity;
    %% Control Matrix
    %pseudo = pinv(error);
    ...
    jointAcceleration = (jacobA'/(jacobA*jacobA' + lambda*eye(6)))*(K*error' + Kd*errorSpeed' - dJacobA*jointSpeed');
    
    jointSpeedPositionControl = (jacobA'/(jacobA*jacobA' + lambda*eye(6)))*K*error';
