...
errorPositionLeft = [0 0 0 0 0 0];
errorPositionRight = [0 0 0 0 0 0];
sumErrorPositionLeft = errorPositionLeft;
sumErrorPositionRight = errorPositionRight;

PoseToGetTo = zeros(1,6);
PoseToGetTo(1:3) = UR5Pose(1,1:3);
PoseToGetTo(4:6) = UR5Pose(1,4:6);
PoseToGetTo(3) = PoseToGetTo(3) + 0.1

while trajectoryCount < 1000
    
    normalVector = (UR5Pose(1,1:3) - UR5Pose(2,1:3))/norm(UR5Pose(1,1:3) - UR5Pose(2,1:3));
    
    % Control Loop
    if (trajectoryStart == 1)
        [UR5JointSpeed(1,:),erroPositionLeft] = control_loop_PID(PoseToGetTo, UR5Pose(1,:), step, handles.clientID, objectScript, URJacobianCall(1), vrep, UR5PoseSpeed(1,:), 0, 0, 0, 0, 0, leftArmDatabaseUR5(trajectoryCount-1,:),errorPositionLeft, sumErrorPositionLeft,0);%forceControl,UR5_1_Force(4:6), speed);
    else
        [UR5JointSpeed(1,:),erroPositionLeft] = control_loop_PID(PoseToGetTo, UR5Pose(1,:), step, handles.clientID, objectScript, URJacobianCall(1), vrep, UR5PoseSpeed(1,:), 0, 0, 0, 0, 0, leftArmDatabaseUR5(trajectoryCount-1,:),errorPositionLeft, sumErrorPositionLeft,0);
    end
    %[UR5JointSpeed(2,:),erroPositionRight] = control_loop_PID(rightArmDatabaseUR5(trajectoryCount,:), UR5Pose(2,:), step, handles.clientID, objectScript, URJacobianCall(2), vrep, UR5PoseSpeed(2,:), 0, 0, 0, 0, 0, rightArmDatabaseUR5(trajectoryCount-1,:),errorPositionRight, sumErrorPositionRight);
    
    sumErrorPositionLeft = sumErrorPositionLeft + errorPositionLeft;
    sumErrorPositionRight = sumErrorPositionRight + errorPositionRight;
...
mkdir(filename);
filename = strcat(filename,'/DLS_V2.mat');
