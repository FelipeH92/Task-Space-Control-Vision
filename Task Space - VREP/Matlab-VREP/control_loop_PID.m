function [jointSpeed,error, w, lambda] = control_loop_PID(trajectoryStep,poseActual, stepSize, clientID, object, functionCall, vrep, endEffectorVelocity, normalVector, forceReading, forceControl,torqueVector, maxSpeed, previousTrajectory, previousError, sumError, trajectorySpeed)% posePrevious)
    %% Error Calculation
    %poseActual
    %trajectoryStep
...
%     end
    ...
    error_d = (error - previousError)/stepSize;
    
    error_i = sumError + error;
...
%     K(6,6) = 1; 
%     
    Kd = 0*eye(6);%Kd = 1*eye(6);
    
    Ki = 0;%0.25*eye(6);%Ki = 0.05*eye(6);
    
    
    K = 0.1*eye(6);%0.1*eye(6);
...
%     error_d'
%     Ki*error_i'
    %jointSpeedPositionControl = (jacobA'/(jacobA*jacobA' + lambda*eye(6)))*(K + (1/(lambdaControl*lambdaControl))*eye(6))*(Kp*error' + Kd*error_d' + Ki*error_i');
    jointSpeedPositionControl = (jacobA'/(jacobA*jacobA' + lambda*eye(6)))*(Kp*error' + Kd*error_d' + Ki*error_i'); %DLS
    %jointSpeedPositionControl = inv(jacobA)*(Kp*error' + Kd*error_d' + Ki*error_i'); %inv
    %jointSpeedPositionControl = pinv(jacobA)*(Kp*error' + Kd*error_d' + Ki*error_i'); %pinv
    %jointSpeedPositionControl = (jacobA'/(jacobA*jacobA' + lambda*eye(6)))*(Kp*error' + trajectorySpeed'); %Feedforward
