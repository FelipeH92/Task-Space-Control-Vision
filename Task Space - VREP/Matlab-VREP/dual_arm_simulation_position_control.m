...
UR5_1_Pose_Speed_Abs = [];
UR5_2_Pose_Speed_Abs = [];

UR5_1_Pose_Acceleration = [];
UR5_2_Pose_Acceleration = [];

UR5_1_Pose_Acceleration_Abs = [];
UR5_2_Pose_Acceleration_Abs = [];
...
while trajectoryCount < length(leftArmDatabaseUR5)
    ...
    if (trajectoryStart == 1)
        [UR5JointSpeed(1,:),erroPositionLeft, w1] = control_loop(leftArmDatabaseUR5(trajectoryCount,:),UR5Pose(1,:),step,handles.clientID, objectScript, URJacobianCall(1), vrep, UR5PoseSpeed(1,:), normalVector, 0, 0, 0, 0,leftArmDatabaseUR5(trajectoryCount-1,:));%forceControl,UR5_1_Force(4:6), speed);
    else
        [UR5JointSpeed(1,:),erroPositionLeft, w1] = control_loop(leftArmDatabaseUR5(trajectoryCount,:),UR5Pose(1,:),step,handles.clientID, objectScript, URJacobianCall(1), vrep, UR5PoseSpeed(1,:), normalVector, 0, 0, 0, 0,leftArmDatabaseUR5(trajectoryCount-1,:));
    end
    [UR5JointSpeed(2,:),erroPositionRight, w2] = control_loop(rightArmDatabaseUR5(trajectoryCount,:),UR5Pose(2,:),step,handles.clientID, objectScript, URJacobianCall(2), vrep, UR5PoseSpeed(2,:), 0, 0, 0,0,0,rightArmDatabaseUR5(trajectoryCount-1,:));
...
figure(5)
plot(SimulationTime,w1Database);
...
figure(6)
plot(SimulationTime,w2Database);
...
figure(13)
plot(SimulationTime,UR5_1_Pose_Speed_Abs);
...
figure(14)
plot(SimulationTime,UR5_2_Pose_Speed_Abs);
...
figure(15)
plot(SimulationTime,UR5_1_Pose_Acceleration_Abs);
...
figure(16)
plot(SimulationTime,UR5_2_Pose_Acceleration_Abs);
